...
export default function ProjectDescription({ title, description, techStack = [], url, className, isReversed }) 
{
  ...
  return (
    <div className={`flex flex-col gap-3.5 max-w-full ${alignmentClass} ${className}`}>
      <h4 className='font-mono text-primary text-sm'>Featured Project</h4>
      <h3 className='font-bold text-xl'>{title}</h3>

      <p className='text-muted-foreground p-5 border rounded-2xl text-sm leading-relaxed'>
        {description}
      </p>

      {/* Tags: flex-row on mobile, flex-row-reverse on lg screens if reversed */}
      <div className={`flex gap-2 flex-wrap ${isReversed ? "flex-row lg:flex-row-reverse" : "flex-row"}`}>
        {techStack.map((tech, index) => 
        {
          return (
            <span 
              key={index} 
              className="font-mono text-muted-foreground text-xs font-light px-3.5 py-0.5 rounded-xl bg-muted/60"
            >
              {tech}
            </span>
          );
